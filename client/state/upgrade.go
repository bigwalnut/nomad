...
// markUpgraded adds version metadata to BoltDB to mark it as upgraded and
// should be run at the end of the upgrade transaction.
func markUpgraded(tx *bolt.Tx) error {
	...
	if err := bkt.Put(metaVersionKey, metaVersion); err != nil {
		return err
	}

	// Write the time the upgrade was done
	return bkt.Put(metaUpgradedKey, []byte(time.Now().Format(time.RFC3339)))
}
...
//	     - mutable (1294 bytes)
//
func UpgradeSchema(logger hclog.Logger, tx *boltdd.Tx) error {
	btx := tx.BoltTx()
	...
	for _, allocBucket := range allocBuckets {
		...
		if err := upgradeAllocBucket(allocLogger, tx, bkt, allocID); err != nil {
			return fmt.Errorf("error upgrading alloc %q state: %v", allocID, err)
		}
	}

	// Mark the database as upgaded.
	return markUpgraded(tx.BoltTx())
}
...
func upgradeAllocBucket(logger hclog.Logger, tx *boltdd.Tx, bkt *bolt.Bucket, allocID string) error {
	...
	//TODO If alloc is not found should we error or delete this bucket?
	if !allocFound {
	...
	for _, taskBucket := range taskBuckets {
		...
		if err != nil {
			// Add task name to error since its not exposed to
			// upgradeTaskBucket
			return fmt.Errorf("error upgrading task %q state: %v", taskName, err)
		}
...
func upgradeTaskBucket(logger hclog.Logger, bkt *bolt.Bucket) (*taskRunnerState08, error) {
	...
	//TODO If simple-all is not found should we error or delete the alloc bucket?
	if !simpleFound {
